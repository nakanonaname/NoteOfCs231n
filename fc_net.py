    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == 'batchnorm':
                ...
                gamma = self.params['gamma' + str(i+1)]
                beta = self.params['beta' + str(i+1)]
                out, ln_cache = layernorm_forward(out, gamma, beta, self.bn_params[i])
                cache = (fc_cache, ln_cache)
            else:
                cache = fc_cache
    
            # ReLU
            out, relu_cache = relu_forward(out)
            ache = (cache, relu_cache) if self.normalization else (fc_cache, relu_cache)
        ...
        dout, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = \
            affine_backward(dscores, caches[-1])
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            if self.normalization == 'batchnorm':
                fc_cache, bn_cache = inner_cache
                dout, dgamma, dbeta = batchnorm_backward(dout, bn_cache)
                grads['gamma' + str(i+1)] = dgamma
                grads['beta' + str(i+1)] = dbeta
            elif self.normalization == 'layernorm':
                fc_cache, ln_cache = inner_cache
